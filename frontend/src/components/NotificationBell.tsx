...
import { Bell, X, Heart, MessageSquare, AtSign, CornerDownRight, Megaphone } from "lucide-react";
...
function NotifIcon({ type }: { type: string }) {
  ...
  if (type === "like") return <Heart className={base} style={{ color: "var(--kvis-purple)" }} />;
  if (type === "comment") return <MessageSquare className={base} style={{ color: "var(--kvis-green-light)" }} />;
  if (type === "reply") return <CornerDownRight className={base} style={{ color: "var(--kvis-green-light)" }} />;
  if (type === "mention") return <AtSign className={base} style={{ color: "oklch(70% 0.18 55)" }} />;
        ...
        <>
          ...
          <div className="fixed right-3 top-16 z-[60] w-[calc(100vw-1.5rem)] max-w-80 border border-[var(--kvis-border)] bg-background shadow-xl rounded-sm overflow-hidden">
            ...
            <div className="max-h-80 overflow-y-auto divide-y divide-[var(--kvis-border)]">
              {notifs.length === 0 ? (
                <p className="px-4 py-8 text-xs text-center text-muted-foreground">No notifications yet.</p>
              ) : (
                notifs.map(n => (
                  <button
                    key={n.id}
                    ...
                    style={{ background: n.is_read ? "transparent" : "var(--kvis-purple-soft)" }}
                  >
                    ...
                    {!n.is_read && (
                      <div className="mt-1.5 shrink-0 w-2 h-2 rounded-full bg-[var(--kvis-purple)]" />
                    )}
                  </button>
                ))
              )}
