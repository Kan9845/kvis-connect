...
export const userApi = {
  ...
  updateProjects: (items: { title: string; advisor?: string; advisor2?: string; description?: string; status: string; link?: string; is_public?: boolean }[]) =>
    api.put("/api/users/me/projects", items).then((r) => r.data),
  updatePublications: (items: { citation: string; doi?: string; is_public?: boolean }[]) =>
    api.put("/api/users/me/publications", items).then((r) => r.data),
  updatePortfolioLinks: (items: { type: string; url: string; is_public?: boolean }[]) =>
  ...
  updateLaunches: (items: { name: string; innovation_type: string; innovation_type_other?: string; role: string; status?: string; description?: string; link?: string; is_public?: boolean }[]) =>
...
export const blogApi = {
  ...
  create: (data: { title: string; content: string; excerpt?: string; cover_image_url?: string | null; tags?: string; is_published: boolean; visibility?: string }) =>
    api.post<BlogDetail>("/api/blogs", data).then((r) => r.data),
  update: (slug: string, data: Partial<{ title: string; content: string; excerpt: string; cover_image_url: string | null; tags: string; is_published: boolean; visibility: string }>) =>
