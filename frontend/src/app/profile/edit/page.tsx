...
export default function EditPage() {
  ...
  useEffect(() => {
    if (me) {
      setExtraContacts(me.extra_contacts ?? []);
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors, isDirty, isSubmitting } } = useForm<GeneralForm>({
    ...
    values: me ? {
      ...
      nickname: me.nickname ?? "", nickname_public: me.nickname_public ?? true,
      kvis_year: me.kvis_year ?? undefined,
      current_status: me.current_status ?? "",
      place: me.place ?? "", place_level2: me.place_level2 ?? "",
      country: me.country ?? "", bio: me.bio ?? "", mbti: me.mbti ?? "",
      zodiac: me.zodiac ?? "", chronotype: me.chronotype ?? "",
      interests: me.interests ?? "", interests_public: me.interests_public ?? true,
      contact_email: me.contact_email ?? "", contact_email_public: me.contact_email_public ?? true,
      facebook_url: me.facebook_url ?? "", linkedin_url: me.linkedin_url ?? "",
      instagram_url: (me as any).instagram_url ?? "",
      line_id: me.line_id ?? "", website_url: me.website_url ?? "",
    } : undefined,
  });
      ...
      <div className="mx-auto max-w-5xl px-6 lg:px-10 py-10 lg:py-14">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as Tab)} className="w-full">
          <TabsList className="h-auto w-full justify-start gap-7 rounded-none border-b border-[var(--kvis-rule)] bg-transparent p-0 pt-5 pb-1 overflow-x-auto">
            {(["general", "education", "career", "research", "personal"] as const).map((t) => (
