...
function SectionHead({ numeral, label, hint }: { numeral: string; label: string; hint?: string }) {
  return (
    <div>
      <div className="flex items-baseline gap-3 md:block">
        <p className="font-mono text-2xl md:text-3xl font-black tabular-nums leading-none" style={{ color: P.purple }}>{numeral}</p>
        <p className="text-xs font-bold uppercase tracking-[0.26em] text-foreground md:mt-3">{label}</p>
      </div>
      {hint && <p className="mt-2 md:mt-3 text-xs italic leading-snug max-w-[24ch]" style={{ color: P.text3 }}>{hint}</p>}
    </div>
  );
}
    ...
    <div className="min-h-full bg-background">
      <div className="mx-auto max-w-4xl px-6 lg:px-10 py-xl lg:py-layout">
        <header className="pb-8 border-b-2 border-[var(--sep-strong)]">
          ...
          <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-black tracking-[-0.03em] leading-[0.95] text-foreground">Write a post</h1>
          <p className="mt-4 text-sm md:text-base text-muted-foreground max-w-[55ch] leading-relaxed">Share an essay, story, or update with the alumni network. Markdown supported.</p>
        ...
        <form>
          <section className={sectionGrid} style={{ borderColor: P.ruleHeavy }}>
            <SectionHead numeral="01" label="Title" hint="Keep it short - twelve words or fewer." />
            <div>
              <input {...register("title")} placeholder="What's the post about?" className="w-full bg-transparent border-0 text-3xl md:text-4xl lg:text-5xl font-black tracking-[-0.02em] leading-[1.05] text-foreground placeholder:text-muted-foreground/35 focus:outline-none" />
              <FieldError msg={errors.title?.message} />
            </div>
          </section>

          <section className={sectionGrid} style={{ borderColor: P.rule }}>
            <SectionHead numeral="02" label="Summary" hint="A one-sentence pitch, shown beneath the title on the list page." />
            <div>
              <textarea {...register("excerpt")} rows={2} placeholder="A short summary - why someone should read this." className="w-full bg-transparent border-0 text-lg md:text-xl leading-relaxed text-foreground/80 placeholder:text-muted-foreground/45 focus:outline-none resize-none" />
            </div>
          </section>
          ...
          <section className={sectionGrid} style={{ borderColor: P.rule }}>
            <SectionHead numeral="04" label="Tags" hint="Comma-separated. Used to group related posts." />
            <div>
          ...
          <section className="pt-10 lg:pt-12 border-t-2" style={{ borderColor: P.ruleHeavy }}>
            <div className="flex items-end justify-between gap-6 flex-wrap mb-5">
              <div className="flex items-baseline gap-4">
                <span className="font-mono text-2xl md:text-3xl font-black tabular-nums leading-none" style={{ color: P.purple }}>05</span>
                <h2 className="text-xs font-bold uppercase tracking-[0.26em] text-foreground">Body</h2>
              </div>
                ...
                {(["write", "preview"] as const).map((t) => {
                  ...
                  return (
                    <button key={t} type="button" onClick={() => setTab(t)} className="text-xs font-bold uppercase tracking-[0.26em] transition-colors"
                      style={{ color: active ? P.purple : P.text3, textDecoration: active ? "underline" : "none", textDecorationThickness: 2, textUnderlineOffset: 8 }}>
            ...
            <div className="border" style={{ borderColor: P.rule }}>
              <div className="flex items-center justify-between px-4 md:px-5 py-2 border-b text-xs uppercase tracking-[0.22em] tabular-nums" style={{ borderColor: P.rule, color: P.text3 }}>
                ...
                <div className="px-4 md:px-5 py-6 min-h-[500px]">
                  {content.trim() ? (
                    <article className="prose prose-lg max-w-none">
                      {title && <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-black tracking-[-0.03em] leading-[0.95] text-foreground">{title}</h1>}
                      <ReactMarkdown remarkPlugins={[remarkGfm]}>{contentWithMentions}</ReactMarkdown>
                    </article>
                  ) : (
                    <p className="text-center text-xs uppercase tracking-[0.28em] py-20" style={{ color: P.text3 }}>Nothing to preview yet - start writing.</p>
                  )}
                </div>
              )}
            </div>
