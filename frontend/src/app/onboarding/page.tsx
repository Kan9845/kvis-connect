...
const KVIS_COHORTS = Array.from({ length: 9 }, (_, i) => i + 1); // K1 → K9
const GRADES = [
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div
          className="mt-0.5 shrink-0 w-9 h-9 flex items-center justify-center rounded-full border"
          style={{
            borderColor: selected ? P.purple : P.border,
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            style={{
              borderColor: active ? P.purple : P.border,
              color: active ? P.purple : "var(--foreground)",
              background: active ? P.purpleSoft : "transparent",
            }}
          >
            {o.label}

            <span className="block text-xs font-normal text-muted-foreground normal-case tracking-normal mt-0.5">
              {o.sub}
            </span>
          </button>
        );
...
export default function OnboardingPage() {
  ...
  async function handleSubmit() {
    ...
    if (role === "alumni") {
      if (!kvisYear) {
        setError("Please select your KVIS cohort.");
        return;
      }
      ...
      if (!teachStartYear) {
        setError("Please enter your first year teaching at KVIS.");
        return;
      }
      if (!department) {
    ...
    try {
      if (role === "alumni") {
        ...
        await userApi.updateMe({
          ...
          teach_end_year:
            !isCurrentTeacher && teachEndYear
              ? parseInt(teachEndYear)
              : undefined,
            ...
            <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-black tracking-[-0.035em] leading-[0.9] text-foreground max-w-[14ch]">
              {step === "role" ? (
                <>
                  Welcome to
                  <br />
                  <span style={{ color: P.purple }}>KVIS Connect</span>
                </>
              ) : (
                <>
                  Almost
                  <br />
                  <span style={{ color: P.purple }}>there</span>
                </>
              )}
            </h1>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === "details" && (
                ...
                >
                  ...
                  {role === "alumni" && (
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.26em] mb-3 text-foreground">
                        KVIS Cohort
                      </p>
                      <div className="grid grid-cols-5 gap-2">
                        {KVIS_COHORTS.map((k) => (
                          <button
                            key={k}
                            type="button"
                            onClick={() => setKvisYear(k)}
                            className="py-2.5 text-sm font-semibold tabular-nums border transition-all"
                            style={{
                              borderColor: kvisYear === k ? P.purple : P.border,
                              color:
                                kvisYear === k ? P.purple : "var(--foreground)",
                              background:
                                kvisYear === k ? P.purpleSoft : "transparent",
                            }}
                          >
                            K{k}
                          </button>
                  ...
                  {role === "faculty" && (
                    <div className="space-y-6">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-[0.26em] mb-3 text-foreground">
                          Department{" "}
                          <span className="text-[var(--kvis-purple)]">*</span>
                        </p>
                        <div className="grid grid-cols-1 gap-2">
                          {KVIS_DEPARTMENTS.map((d) => (
                            <button
                              ...
                              style={{
                                borderColor:
                                  department === d ? P.purple : P.border,
                                color:
                                  department === d
                                    ? P.purple
                                    : "var(--foreground)",
                      ...
                      <div>
                        <p className="text-xs font-bold uppercase tracking-[0.26em] mb-3 text-foreground">
                          First year teaching at KVIS
                        </p>
                        <input
                          type="number"
                          min={1990}
                          max={new Date().getFullYear()}
                          value={teachStartYear}
                          onChange={(e) => setTeachStartYear(e.target.value)}
                      ...
                      <div>
                        <p className="text-xs font-bold uppercase tracking-[0.26em] mb-3 text-foreground">
                          Still teaching at KVIS?
                        </p>
                        <PillToggle
                          options={[
                            { value: "yes", label: "Yes, currently" },
                            { value: "no", label: "No, I left" },
                          ]}
                      ...
                      {!isCurrentTeacher && (
                        <div>
                          ...
                          <input
                            ...
                            max={new Date().getFullYear()}
                            value={teachEndYear}
                            onChange={(e) => setTeachEndYear(e.target.value)}
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.26em] transition-colors"
                      style={{ color: "var(--kvis-text2)" }}
                    >
